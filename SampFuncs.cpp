...
void onFootUpdateAtNormalPos(CNetGame* netgame)
{
	...
	ofSync.byteHealth = (BYTE)netgame->fPlayerHealth;
	ofSync.byteArmour = (BYTE)netgame->fPlayerArmour;
	//ofSync.fQuaternion[3] = netgame->fNormalModeRot;
	ofSync.vecPos[0] = netgame->fNormalModePos[0];
	ofSync.vecPos[1] = netgame->fNormalModePos[1];
	ofSync.vecPos[2] = netgame->fNormalModePos[2];

	SendOnFootFullSyncData(&ofSync, 0, -1, netgame);

	AIM_SYNC_DATA aimSync;
	memset(&aimSync, 0, sizeof(AIM_SYNC_DATA));

	netgame->playersInfo[netgame->playerID].aimData.byteCamMode = 4;
	netgame->playersInfo[netgame->playerID].aimData.vecAimf1[0] = 0.1f;
	netgame->playersInfo[netgame->playerID].aimData.vecAimf1[1] = 0.1f;
	netgame->playersInfo[netgame->playerID].aimData.vecAimf1[2] = 0.1f;
	netgame->playersInfo[netgame->playerID].aimData.vecAimPos[0] = netgame->fNormalModePos[0];
	netgame->playersInfo[netgame->playerID].aimData.vecAimPos[1] = netgame->fNormalModePos[1];
	netgame->playersInfo[netgame->playerID].aimData.vecAimPos[2] = netgame->fNormalModePos[2];

	SendAimSyncData(0, 0, -1, netgame);
}
...
void onFootUpdateFollow(PLAYERID followID, CNetGame* netgame)
{
	ONFOOT_SYNC_DATA ofSync;
	memset(&ofSync, 0, sizeof(ONFOOT_SYNC_DATA));
	SendOnFootFullSyncData(&ofSync, 0, followID, netgame);
	SendAimSyncData(0, 0, followID, netgame);
}

void inCarUpdateFollow(PLAYERID followID, VEHICLEID withVehicleID, CNetGame* netgame)
{
	if (netgame->playersInfo[netgame->playerID].iAreWeInAVehicle)
	{
		INCAR_SYNC_DATA icSync;
		memset(&icSync, 0, sizeof(INCAR_SYNC_DATA));
		icSync.VehicleID = withVehicleID;
//		SendInCarFullSyncData(&icSync, 0, followID, netgame);
		SendAimSyncData(0, 0, followID, netgame);
	}

	if (netgame->inCarUpdateTick && netgame->inCarUpdateTick < (netgame->GetTickCount() - 10000))
	{
		if (!netgame->playersInfo[netgame->playerID].iAreWeInAVehicle)
		{
			if (netgame->playersInfo[followID].incarData.VehicleID == (VEHICLEID)-1)
				return;

			SendEnterVehicleNotification((VEHICLEID)netgame->playersInfo[followID].incarData.VehicleID, 0, netgame);
			netgame->playersInfo[netgame->playerID].iAreWeInAVehicle = 1;
			netgame->inCarUpdateTick = 0;
		}

		netgame->inCarUpdateTick = netgame->GetTickCount();
	}
	else
	{
		if (!netgame->playersInfo[netgame->playerID].iAreWeInAVehicle)
			onFootUpdateFollow(followID, netgame);
	}
}
...
void spectatorUpdate(CNetGame* netgame)
{
	...
	spSync.vecPos[0] = netgame->fNormalModePos[0];
	spSync.vecPos[1] = netgame->fNormalModePos[1];
	spSync.vecPos[2] = netgame->fNormalModePos[2];

	SendSpectatorData(&spSync, netgame);
...
{
	if (netgame->client == NULL) return;

	RakNet::BitStream bsSpawnRequest;
	bsSpawnRequest.Write(iClass);
	netgame->client->RPC(&RPC_RequestClass, &bsSpawnRequest, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
}
...
void sampSpawn(CNetGame* netgame)
{
	...
	netgame->iLocalPlayerSkin = netgame->SpawnInfo.iSkin;
	netgame->fNormalModePos[0] = netgame->SpawnInfo.vecPos[0];
	netgame->fNormalModePos[1] = netgame->SpawnInfo.vecPos[1];
	netgame->fNormalModePos[2] = netgame->SpawnInfo.vecPos[2];
	netgame->fNormalModeRot = netgame->SpawnInfo.fRotation;

	RakNet::BitStream bsSendRequestSpawn;
	netgame->client->RPC(&RPC_RequestSpawn, &bsSendRequestSpawn, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);

	RakNet::BitStream bsSendSpawn;
	netgame->client->RPC(&RPC_Spawn, &bsSendSpawn, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);

	netgame->bIsSpectating = 0;
}

void sampSpam(CNetGame* netgame)
{
	/*if (GetTickCount() - netgame->dwLastDisconnection >= settings.uiSpamInterval)
	{
...
void sendServerCommand(char* szCommand, CNetGame* netgame)
{
	if (!strnicmp(szCommand + 1, "rcon", 4))
	{
		...
		netgame->client->Send(&bsSend, HIGH_PRIORITY, RELIABLE, 0);
	}
	else
	{
		...
		netgame->client->RPC(&RPC_ServerCommand, &bsParams, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
	}
}
...
void sendChat(char* szMessage, CNetGame* netgame)
{
	...
	netgame->client->RPC(&RPC_Chat, &bsSend, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
}
...
void sendScmEvent(int iEventType, DWORD dwParam1, DWORD dwParam2, DWORD dwParam3, CNetGame* netgame)
{
	...
	netgame->client->RPC(&RPC_ScmEvent, &bsSend, HIGH_PRIORITY, RELIABLE_SEQUENCED, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
}
...
void sendDialogResponse(WORD wDialogID, BYTE bButtonID, WORD wListBoxItem, char* szInputResp, CNetGame* netgame)
{
	...
	netgame->client->RPC(&RPC_DialogResponse, &bsSend, HIGH_PRIORITY, RELIABLE_ORDERED, 0, FALSE, UNASSIGNED_NETWORK_ID, 0);
}
...
void sendPickUp(int iPickupID, CNetGame* netgame)
{
	...
	netgame->client->RPC(&RPC_PickedUpPickup, &bsSend, HIGH_PRIORITY, RELIABLE_ORDERED, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
}
...
void selectTextDraw(int iTextDrawID, CNetGame* netgame)
{
	...
	netgame->client->RPC(&RPC_ClickTextDraw, &bsSend, HIGH_PRIORITY, RELIABLE_ORDERED, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
}
...
int isPlayerConnected(PLAYERID iPlayerID, CNetGame* netgame)
{
	...
	if (!netgame->playersInfo[iPlayerID].iIsConnected)
		return 0;
...
int getPlayerID(char* szPlayerName, CNetGame* netgame)
{
	...
	{
		if (netgame->playersInfo[i].iIsConnected == 0) continue;

		if (!strcmp(netgame->playersInfo[i].szPlayerName, szPlayerName))
			return i;
	}
...
{
	if (netgame->playersInfo[iPlayerID].iIsConnected && iPlayerID >= 0 && iPlayerID <= MAX_PLAYERS)
		return netgame->playersInfo[iPlayerID].szPlayerName;
...
{
	if (!netgame->playersInfo[iPlayerID].iIsConnected) return 0;

	if (iPlayerID > 0 && iPlayerID <= MAX_PLAYERS)
	{
		memcpy(fPos, netgame->playersInfo[iPlayerID].onfootData.vecPos, sizeof(float) * 3);
		return 1;
	}
...
PLAYERID getPlayerIDFromPlayerName(char* szName, CNetGame* netgame)
{
	...
	{
		if (!netgame->playersInfo[i].iIsConnected) continue;
		if (!strcmp(netgame->playersInfo[i].szPlayerName, szName))
			return (PLAYERID)i;
	}
...
unsigned short getPlayerCountWoNPC(CNetGame* netgame)
{
	...
	{
		if (!netgame->playersInfo[i].iIsConnected) continue;
		if (netgame->playersInfo[i].byteIsNPC) continue;
		count++;
	}
	return count;
}
...
void ApplyAnimation(CNetGame* netgame) {
	...
	const char* animLib = "PED";
	const char* animName = "WALK_CIVI";
	float fDelta = 2.0;
	uint32_t time = 0;

	bsData.Write(netgame->playerID);
	bsData.Write(strlen(animLib));
	bsData.Write(animLib, strlen(animLib));
	bsData.Write(strlen(animName));
	bsData.Write(animName, strlen(animName));
	bsData.Write(fDelta);
	bsData.Write(true);
	bsData.Write(false);
	bsData.Write(false);
	...
	netgame->client->RPC(&RPC_ScrApplyAnimation, &bsData, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
}
...
void ClearAnimation(CNetGame* netgame) {
	...
	netgame->client->RPC(&RPC_ScrClearAnimations, &bsData, HIGH_PRIORITY, RELIABLE, 0, FALSE, UNASSIGNED_NETWORK_ID, NULL);
...
int GetAnimIdxByName(const char* szName)
{
	...
	{
		if (!strcmp(szName, g_szAnimBlockNames[i]))
		{
			return i;
		}
	}
